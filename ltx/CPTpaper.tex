...
\end{equation}
where $r(t)$ denotes the IA, $\phi(t)$ denotes the IP and $x_0(t)$ denotes an offset that will be modeled as the circle center. Note, the offset is assumed to be varying on a slow time-scale such that is can be filtered out in a pre-processing step, thus is assumed to be at the origin for clarity of argument. In the noise-free case, the IA is
...
\hat{\phi} \left( n+1,\kappa \right) - \hat{\phi} \left( n,\kappa^* \right) \le \alpha. 
\end{equation}
