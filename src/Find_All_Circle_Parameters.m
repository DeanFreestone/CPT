...
function [x0_temp Hx0_temp r_temp phi_temp x_temp Hx_temp M_temp e d] =  Find_All_Circle_Parameters(MaxPoints, InitialPoints, PointsStep, y_s_all, Hy_s_all, x_previous, Hx_previous)
    ...
    for n=PointsFromCenterInner:PointsStep:PointsFromCenterOuter

        y_s = y_s_all(center-n:center+n);                 % put the y and Hy signal in a more readable form
        Hy_s = Hy_s_all(center-n:center+n);               % this is the segment used to fit the circle
        ...
        x0_temp(m) = Par(1);                                % estimate for y0 translation
        Hx0_temp(m) = Par(2);                               % estimate for Hy0 translation
        r_temp(m) = Par(3);                                 % estimate for radius (envelope)
        
        % might be able to move these guys outside the loop to make it
        % faster (to do later)
%         ~~~~~~~~~~~
        phi_temp(m) = atan2(Hy_s_all(center)-Hx0_temp(m), y_s_all(center)-x0_temp(m));
        x_temp(m) = r_temp(m)*cos(phi_temp(m));
        Hx_temp(m) = r_temp(m)*sin(phi_temp(m));
        
        e(m) = mean( (y_s - x0_temp(m)).^2 + (Hy_s - Hx0_temp(m)).^2 - r_temp(m)^2 );   % estimate the mse for the signal
...
% ~~~~~~~~~~~~~~~~
        ...
        x_previous_s = x_previous(end-n+1:end);           % take the part of the previous estimate that correspond to the segment size
        Hx_previous_s = Hx_previous(end-n+1:end);
        ...
        if ~isempty(x_previous_s_not_nan)
            ...
            y_previous_s = y_s_all(center-n:center-1);
            Hy_previous_s = Hy_s_all(center-n:center-1);
            ...
            d(m) = Relative_Distance_Mismatch(x_temp(m), Hx_temp(m), x_previous_s_not_nan(1), Hx_previous_s_not_nan(1), y_s_all(center), Hy_s_all(center), y_previous_s_not_nan(1), Hy_previous_s_not_nan(1));
